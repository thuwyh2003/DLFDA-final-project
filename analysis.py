...
def plot_R2_bar(R_df, type):
    ...
    ax.bar(x - width*3 , FF, width, label='FF', color=plt.get_cmap('YlGnBu')(np.linspace(0, 1, 9)[1]))
    ax.bar(x - width*2 , PCA, width, label='PCA', color=plt.get_cmap('YlGnBu')(np.linspace(0, 1, 9)[2]))
    ax.bar(x - width , IPCA, width, label='IPCA', color=plt.get_cmap('YlGnBu')(np.linspace(0, 1, 9)[3]))
    ax.bar(x + 0.00, CA0, width, label='CA0', color=plt.get_cmap('YlGnBu')(np.linspace(0, 1, 9)[4]))
    ax.bar(x + width , CA1, width, label='CA1', color=plt.get_cmap('YlGnBu')(np.linspace(0, 1, 9)[5]))
    ax.bar(x + width*2 , CA2, width, label='CA2', color=plt.get_cmap('YlGnBu')(np.linspace(0, 1, 9)[6]))
    ax.bar(x + width*3 , CA3, width, label='CA3', color=plt.get_cmap('YlGnBu')(np.linspace(0, 1, 9)[7]))
    ax.bar(x + width*4 , VAE, width, label='VAE', color=plt.get_cmap('YlGnBu')(np.linspace(0, 1, 9)[8]))
...
if __name__=="__main__":
    # total_R2 = calculate_R2("VAE_wzy_3", 'inference')
    # print(total_R2)
    # predict_R2 = calculate_R2("VAE_wzy_3", 'predict')
    # print(predict_R2)
    ...
    FFs = ["FF_1", "FF_2", "FF_3", "FF_4", "FF_5"]
    PCAs = ["PCA_1", "PCA_2", "PCA_3", "PCA_4", "PCA_5"]
    IPCAs = ["IPCA_1", "IPCA_2", "IPCA_3", "IPCA_4", "IPCA_5"]
    VAEs = ["VAE_wzy_1", "VAE_wzy_2", "VAE_wzy_3", "VAE_wzy_4", "VAE_wzy_5"]
    models = FFs + PCAs + IPCAs + CAs + VAEs
    # models = CAs
    ...
    ## Save R^2 tables
    R_total_df = pd.DataFrame(np.array(total_R2).reshape(-1, 5), columns = ['K=1', 'K=2', 'K=3', 'K=4', 'K=5'], index=['FF', 'PCA', 'IPCA', 'CA0', 'CA1', 'CA2', 'CA3', 'VAE'])
    R_pred_df = pd.DataFrame(np.array(predict_R2).reshape(-1, 5), columns = ['K=1', 'K=2', 'K=3', 'K=4', 'K=5'], index=['FF', 'PCA', 'IPCA', 'CA0', 'CA1', 'CA2', 'CA3', 'VAE'])
